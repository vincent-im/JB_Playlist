...
st.divider() # 코드 내부의 유효하지 않은 구분선을 st.divider()로 수정했습니다.
...
st.divider() # 이 부분의 구분선도 안전하게 수정했습니다.
...
if not st.session_state.playlist_items:
    ...
    st.divider() # 이 부분도 정상 수정 완료되었습니다.
    
    # 최종 반영 버튼
    if st.button("✨ Playlist에 반영", type="primary", use_container_width=True):
        st.info("유튜브 API 연동 및 플레이리스트 반영 작업을 시작합니다...")
